...
class MainWindow(QMainWindow):
    ...
    def _show_export_viewer(self):
        ...
        combo = QComboBox()
        combo.addItem("综合 — singbox.json (纯数组，GUI.for.SingBox 导入)", "singbox.json")
        ...
        combo.addItem("纯 HTTP — singbox.json", "http/singbox.json")
        ...
        combo.addItem("⭐ 测速结果 — singbox.json (只含实测过的)", "speedtest/singbox.json")
        combo.addItem("⭐ 测速结果 — clash.yaml", "speedtest/clash.yaml")
        combo.addItem("⭐ 测速结果 — v2ray.txt", "speedtest/v2ray.txt")
        combo.addItem("⭐ 测速结果 — nodes.txt", "speedtest/nodes.txt")
